...
public class WordList : MonoBehaviour
{
    ...
    // ¬се сопрограммы возвращают значени€ типа IEnumerator
    public IEnumerator ParseLines()
    {
        ...
        for (currLine = 0; currLine < totalLines; currLine++)
        {
            ...
            if (currLine % numToParseBeforeYield == 0)
            {
                ...
                // ѕриостановить выполнение сопрограммы до следующего кадра
                yield return null;
                // yield return new WaitForSeconds(1); // приостановить выполнение не менее
                   //  чем на секунду
                // повтор€ющиес€ действи€ через определЄнные интервалы времени
                // можно выполн€ть с применением метода InvokeRepeating()

                // »нструкци€ yield приостановит выполнение этого метода,
                //   даст возможность выполнитьс€ другому коду и возобновит
        ...
        // ѕослать игровому объекту GameObject сообщение об окончании анализа
